...
class CharEncodingCheck(ICiBuildPlugin):
    ...
    def RunBuildPlugin(self, packagename, Edk2pathObj, pkgconfig, environment, PLM, PLMHelper, tc, output_stream=None):
        ...
        abs_pkg_path = Edk2pathObj.GetAbsolutePathOnThisSystemFromEdk2RelativePath(packagename) # MU_CHANGE
        ...
        for (ext, enc) in EcodingMap.items():
            ...
            files = [Edk2pathObj.GetAbsolutePathOnThisSystemFromEdk2RelativePath(x) for x in files] # MU_CHANGE
            for a in files:
                files_tested += 1
